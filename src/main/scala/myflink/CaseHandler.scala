...
import Functions.KafkaMsg
import org.apache.flink.configuration.Configuration
import org.apache.flink.metrics.Counter
import org.apache.flink.streaming.api.functions.ProcessFunction
import org.apache.flink.util.Collector

class CaseHandler extends ProcessFunction[KafkaMsg, KafkaMsg]{
  ...
  override def processElement(i: KafkaMsg, context: ProcessFunction[KafkaMsg, KafkaMsg]#Context, collector: Collector[KafkaMsg]): Unit = {
    eventCounter.inc()
    i.value.split(" ").map(data => collector.collect(KafkaMsg("", data.toUpperCase())))
  }
